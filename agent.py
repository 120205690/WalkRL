...
class Agent:
    def __init__(self, env, path_save=PATH_SAVE, path_load=PATH_LOAD, actor_lr=ACTOR_LR, critic_lr=CRITIC_LR, gamma=GAMMA, tau=TAU, reward_scale=REWARD_SCALE):
        ...
        self.actor = Actor(actions_dim=self.actions_dim, name='actor', upper_bound=env.action_space.high, beta=self.actor_lr)
        self.critic_0 = Critic(name='critic_0', beta=self.critic_lr)
        self.critic_1 = Critic(name='critic_1', beta=self.critic_lr)
        self.critic_value = CriticValue(name='value', beta=self.critic_lr)
        self.critic_target_value = CriticValue(name='target_value', beta=self.critic_lr)
    ...
    def update_target_networks(self, tau):
        ...
        for name in critic_value_params:
            critic_value_params[name] = tau*critic_value_params[name].clone() + \
                    (1-tau)*critic_target_value_params[name].clone()
    ...
    def learn(self):
        ...
        old_q_value_0 = torch.squeeze(self.critic_0(state, action), 1)
        old_q_value_1 = torch.squeeze(self.critic_1(state, action), 1)
